...
class UserController extends Controller
{
    ...
    public function home()
    {
        ...
        if ($user) {
            // check if we want to update rank or not
            // $totalReferral = User::where('ref_by',$user->id)->count();
            $totalReferral = 28;
            $points = $user->points;

            if ($points == 300 && $totalReferral == 8) {
                //update the rank here
                $user->rank = 'Bronze';
            } else if ($points == 800 && $totalReferral == 16) {
                //update the rank here
                $user->rank = 'Silver';
            } else if ($points == 2000 && $totalReferral == 30) {
                //update the rank here
                $user->rank = 'Gold';
            } else if ($points == 7200) {
                //update the rank here
                $user->rank = 'Ass. Supervisor';
            } else if ($points == 21600) {
                //update the rank here
                $user->rank = 'Supervisor';
            } else if ($points == 70000) {
                //update the rank here
                $user->rank = 'Ass. Manager';
            } else if ($points == 200000) {
                //update the rank here
                $user->rank = 'Global Manager';
            } else if ($points == 530000) {
                //update the rank here
                $user->rank = 'Eagle Manager';
            } else if ($points == 650000) {
                //update the rank here
                $user->rank = 'Soaring Manager';
            }
            $user->save();
    ...
    public function buyPlan(Request $request)
    {
        ...
        $EightyPercentPoints = $plan->price * 80 / 100;
        ...
        $gnl = GeneralSetting::first();
        if ($gnl->ref_upgr == 1) {
                levelCommision($user->id, $plan->price, $commissionType = 'Subscription Comission', $EightyPercentPoints);
            }
    ...
    public function pointsCommissions(){
        $page_title = "Points Commissions";
        $commissions = PointLog::where('user_id',auth()->user()->id)->with('userFrom')->paginate(getPaginate());
        return view(activeTemplate().'user.points-commissions',compact('page_title','commissions'));
    }
    ...
    public function referredUsers(){
        ...
        $levels = Referral::get();
        $user = auth()->user();
        $isPlanPurchased = $user->plan_id;
        // User::where('ref_by',auth()->user()->id)->count()
        return view(activeTemplate().'user.referred',compact('page_title','refUsers','isPlanPurchased','levels','user'));
    }
